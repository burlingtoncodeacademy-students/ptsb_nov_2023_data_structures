...
let deceasedPresidents = presidents.filter(
  (president) => president.passed !== undefined
);
...
presidents.forEach((pres) => {
  if (pres.passed !== undefined) {
    let ageDiff = pres.passed - pres.year;
    console.log(`${pres.first} ${pres.last} was ${ageDiff} when they passed.`);
  }
});
